...
- 정리 시작 화면에 필터 시작, 필터 없는 즉시 시작, 시작 위치 선택(피커) 동선을 명확히 분리했다.
- 스와이프 카드에 결정 틴트(삭제/보관/즐겨찾기), 메타데이터 칩(타입/촬영일/길이/용량), 보더/제스처 처리 개선을 반영했다.
...
- 설정에서 `일일 목표`를 제거하고, `UserPreference`/Repository/ViewModel 도메인에서 관련 필드를 정리했다. 설정은 테마/언어/햅틱 중심으로 단순화했다.
